...
@Component({
  selector: 'app-modal-beneficio',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './modal-beneficio.component.html',
  styleUrl: './modal-beneficio.component.css'
})
export class ModalBeneficioComponent implements AfterViewInit, OnInit{
  ...
  async incrementalGeocode(address: string) {
    ...
    validateAndConfirm() {
      ...
      if (this.isValidLocation && this.motivo) {
        this.clienteService.currentUser.subscribe(user => {       
          this.usuario = user.nombreUsuario;
        });
        if (this.usuario) {
          this.clienteService.elegirTipoDireccionBeneficio(this.usuario, this.motivo, this.selectedAddress);
 ...
 export default ModalBeneficioComponent;